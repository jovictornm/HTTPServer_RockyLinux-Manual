...
No seu Virtual Host ou httpd.conf:
...
Instale o módulo de headers (geralmente já vem instalado):
...
Se usar diretórios personalizados, ajuste o contexto do SELinux:
...
Permitir que Apache faça conexões de rede (se necessário):
...
No httpd.conf ou no Virtual Host:
...
### Configurar MPM (Multi-Processing Module)
...
Escolha entre prefork, worker ou event. Para event (recomendado):
...
### Habilitar Compressão (mod_deflate)
...
# Logs personalizados (Virtual Hosts)
/var/log/httpd/site1.com_access.log
/var/log/httpd/site1.com_error.log
```
...
### Analisar Logs com AWK

Mostrar IPs mais frequentes:

```bash
sudo awk '{print $1}' /var/log/httpd/access_log | sort | uniq -c | sort -nr | head -10
```

Mostrar páginas mais acessadas:

```bash
sudo awk '{print $7}' /var/log/httpd/access_log | sort | uniq -c | sort -nr | head -10
```

### Rotação de Logs

O Rocky Linux usa logrotate por padrão. Verifique a configuração:
